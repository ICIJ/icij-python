...
def test_rename_nested_field():
    assert rename_field({"foo": {"bar": "baz"}}, "bar", "qux") == {
        "foo": {"qux": "baz"}
    }
...
def test_rename_nested_list_field():
    assert rename_field(
        {"foo": [{"key": "val1"}, {"key": "val2"}]}, "key", "new_key"
    ) == {"foo": [{"new_key": "val1"}, {"new_key": "val2"}]}


def test_rename_nested_list_field_with_non_homogeneous_list():
    assert rename_field({"foo": ["erk!", {"key": "val1"}]}, "key", "new_key") == {
        "foo": ["erk!", {"new_key": "val1"}]
    }
...
def test_move_field():
    ...
    assert move_field({"foo": "bar"}, "foo", "level1.level2.foo") == {
...
def test_move_field_with_existing_siblings():
    assert move_field(
        {"foo": "bar", "level1": {"baz": "qux"}}, "foo", "level1.foo"
    ) == {"level1": {"foo": "bar", "baz": "qux"}}
...
def test_get_date_from_task_for_batch_download():
    ...
    ) == datetime.datetime.fromisoformat("2024-08-20T07:27:42.192")
...
def test_get_date_from_task_for_batch_search():
    date = get_date_from_task(
        {
            "@type": "Task",
            "id": "771c45b4-1fac-421f-9791-64ac4e1eb4ab",
            ...
            "result": 5,
            "args": {"@type": "java.util.Collections$UnmodifiableMap"},
        }
    )
    ...
    date = get_date_from_task(
        {
            ...
            "id": "771c45b4-1fac-421f-9791-64ac4e1eb4ab",
            "name": "my_task",
            "createdAt": a_date.timestamp() * 1000,
        }
    )
    assert date == a_date
...
def test_change_value():
    assert change_value({"foo": "bar"}, "foo", lambda v: v + "_blah") == {
        "foo": "bar_blah"
    }
    assert change_value({"foo": {"level1": 1}}, "foo.level1", lambda v: v + 1) == {
        "foo": {"level1": 2}
    }
    assert change_value(
        {"foo": {"level1": 1}, "bar": "baz"}, "foo.level1", lambda v: v + 1
    ) == {"foo": {"level1": 2}, "bar": "baz"}
...
def test_change_value_with_list():
    assert change_value(
        {"foo": {"level1": [{"field": 1}, {"field": 2}]}},
        "foo.level1[].field",
        lambda v: v + 1,
    ) == {"foo": {"level1": [{"field": 2}, {"field": 3}]}}
